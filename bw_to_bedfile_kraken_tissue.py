...
Tissue_index = pd.read_csv('%s/Input/%s_index.txt' %(basic_path,sys.argv[2]), sep='\t',header=None, index_col=None)
Tissue_info = pd.read_csv('%s/Input/%s_info.txt' %(basic_path,sys.argv[2]), sep='\t',header=None, index_col=None)
#------------------------------------------------
#test_path = '/Users/yujijun/Documents/01-Work/02-HiglassProject/01-DataFilter/Output_data/'
#seletedsample = pd.read_csv("%s/seletedsample_recluster(222).txt" %test_path, sep='\t',header=None, index_col=None)
#allchrinfo = pd.read_csv("%s/chromInfo.txt.allchr" %test_path, sep='\t',header=None, index_col=None)


#all bw name list:
sample_list = Tissue_info.iloc[:,0].tolist()
Tissue_index_list = Tissue_index.iloc[:,0].tolist()
bw_name = [str(sampleid) + "_treat.bw" for sampleid in Tissue_index_list]
#print(bw_name)
#all chr name list:
...
for a in range(len(allchrname_list)):
    ...
    #lasthalf_empty
    df_lasthalf_empty = pd.DataFrame()
    print(df_lasthalf_empty.shape)
    for b in range(len(bw_name)):
        ...
        if (chromo in bw.chroms().keys()):
            ...
            #print(type(chromo_values_split16_mean_norm))
            df_lasthalf_empty[b] = chromo_values_split16_mean_norm
            bw.close()
            print("\t")
        else:
            print("%s is not in the %s" %(chromo, bw))
            df_lasthalf_empty[b] = [0]*math.ceil(length_chromo/resolution)
            bw.close()
            print("\t")
            #concat two dataframe
    df_lasthalf_empty.columns = sample_list
    ...
    df_merge.to_csv('%s/bedfile_1000_%s%s_%s.csv' %(Output_path,sys.argv[2],len(sample_list),chromo), sep='\t',index=False, header=True)
